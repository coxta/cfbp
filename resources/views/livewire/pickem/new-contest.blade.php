<div x-data="{ creating: false }" @create-contest.window="creating = true" x-show="creating" class="relative z-50"
    ...
    <div class="fixed inset-0 z-50 w-screen overflow-y-auto">
        ...
        -->
            <div
                class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all my-8 w-full max-w-xl p-6">
                <div class="flex flex-row items-center justify-between">
                    <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">Create a Contest</h3>
                    <button @click="creating = false" type="button"
                ...
                <div class="flex flex-col space-y-6 mt-6 w-full text-left">
                    ...
                    @foreach ($typeOptions as $option)
                        <x-radio id="{{ $option['value'] }}" label="{{ $option['name'] }}"
                            wire:model.live="contest.type_id" value="{{ $option['value'] }}" />
                    @endforeach
                </div>
